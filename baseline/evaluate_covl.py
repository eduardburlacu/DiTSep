# Script taken from https://github.com/facebookresearch/denoiser/blob/fix_ta/scripts/matlab_eval.py
import argparse
import json
import os
import sys
...
from src.evaluate_mp import summarize
...
def lpcoeff(speech_frame, model_order):
    ...
    for i in range(model_order):
        ...
        rcoeff[i] = (R[i + 1] - sum_term) / np.maximum(1e-15, E[i])
        a[i] = rcoeff[i]
        if i > 0:
...
def llr(ref_wav, deg_wav, srate):
    ...
    for frame_count in range(num_frames):
        ...
        numerator = np.maximum(
            1e-10, A_processed.dot(toeplitz(R_clean)).dot(A_processed.T)
        )
        denominator = np.maximum(1e-10, A_clean.dot(toeplitz(R_clean)).dot(A_clean.T))
...
def filename_tgt2enh(tgt_path, enhanced_path):
    sample_idx, channel_idx = tgt_path.stem.split(".")
    sample_idx = int(sample_idx)
    channel_idx = int(channel_idx[3:])
    enh_path = enhanced_path / f"{sample_idx:04d}.enh{channel_idx}.wav"
    return sample_idx, channel_idx, enh_path


def compute_metrics(sample_idx, channel_idx, clean_path_f, enhanced_path_f, sr):
    clean_sig = librosa.load(clean_path_f, sr=sr)[0]
    enhanced_sig = librosa.load(enhanced_path_f, sr=sr)[0]
    res = eval_composite(clean_sig, enhanced_sig)
    return sample_idx, channel_idx, res
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute COVL, CSIG, and CBAK metrics from Hu and Loizou"
    )
    parser.add_argument("clean_path", type=Path, help="Path to folder of clean files")
    parser.add_argument(
        "enhanced_path", type=Path, help="Path to folder of enhanced files"
    )
    ...
    split = enhanced_path.name
    output_path = args.enhanced_path.parents[1]

    with utils.ProcessingPool(os.cpu_count()) as pool:
        for clean_path_f in tqdm(clean_path.rglob("*.wav")):
            sample_idx, channel_idx, enhanced_path_f = filename_tgt2enh(
                clean_path_f, enhanced_path
            )
            pool.push(
                compute_metrics,
                (sample_idx, channel_idx, clean_path_f, enhanced_path_f, sr),
            )
        results, _ = pool.wait_results(progress_bar=True)

    csig, cbak, covl, count = 0, 0, 0, 0
    results = sorted(results, key=lambda x: tuple(x[:2]))
    output = {}
    for sample_idx, channel_idx, res in results:
        if sample_idx in output:
            for met in ["csig", "cbak", "covl"]:
                output[sample_idx][met].append(res[met])
        else:
            output[sample_idx] = {}
            for met in ["csig", "cbak", "covl"]:
                output[sample_idx][met] = [res[met]]

    summary = summarize(output)
    print(summary)

    with open(output_path / f"{split}_covl.json", "w") as f:
        json.dump(output, f, indent=2)
    with open(output_path / f"{split}_summary_covl.json", "w") as f:
