...
split_map = {
    ...
    "train": "tr",
    "librimix_test": "test",
    "librimix_train-100": "train-100", 
    "librimix_train-360": "train-360",
    "librimix_dev": "dev",
}
...
class WSJ0_mix(torch.utils.data.Dataset):
    def __init__(
        ...
        split: Optional[str] = "librimix_train-100",
        max_len_s: Optional[float] = None,
        max_n_samples: Optional[int] = None,
        mix_dir: Optional[str] = "mix_clean",
    ):
        super().__init__()
...
class WSJ0_mix_Module(pl.LightningDataModule):
    ...
    def setup(self, *args, **kwargs):
        for split in self.cfg.datamodule:
            if 'libri' not in split: 
                continue
            test_mod = instantiate(self.cfg.datamodule[split]['dataset'])
    ...
    def train_dataloader(self):
        return self._get_dataloader("librimix_train-360")

    def val_dataloader(self):
        return self._get_dataloader("librimix_dev")

    def test_dataloader(self):
        return self._get_dataloader("librimix_test")
